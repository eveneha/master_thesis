...
# ——— load your data once ———
inputs = np.load("input.npy").astype(np.int8)   # (N,1000,1,1)
...
for i in range(5): #total
    ...
    print(f"\n--- Batch {i} ---")
    print(f"Input sample (first 10 values): {x[0, :10, 0, 0]}") # See the input that generated this output
    print(f"Raw output array shape: {out.shape}")
    print(f"Raw output array dtype: {out.dtype}")
    print(f"Raw output array (flattened): {out.flatten()}")
