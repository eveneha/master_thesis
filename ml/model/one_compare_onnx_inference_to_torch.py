...
ONNX_MODEL_PATH = "/home/eveneiha/finn/workspace/finn/onnx/20_tcn_after_oart.onnx" # <--- PATH TO ONNX TO TEST
    ...
    if num_to_run == 0:
        ...
        for i in range(num_to_run):
            ...
            try:
                ...
                current_sample = current_sample#[:,168:833,:] # 0-based index
                if current_sample.ndim == 1 and current_sample.shape[0] == 1000:
                    ...
                    try:
                         ...
                         raise ValueError(f"Cannot reshape input sample {i} (shape {current_sample.shape}) to target (1, 1, 665, 1)")
            ...
            try:
                ...
                if MODEL_CHOICE == "inference" and apply_input_slice:
                    onnx_input_nchw = pytorch_input_np#[:, :, 168:833, :]
                ...
                # onnx_sim_input_nhwc = np.transpose(onnx_input_dict, (0, 2, 3, 1))
                # onnx_sim_input = np.ascontiguousarray(onnx_sim_input_nhwc) #
            ...
            try:
                ...
                #print(f"PyTorch Output (int32): {pytorch_output_np}")
                #print(f"ONNX Output (int32): {onnx_output_np}")
                onnx_output_float = onnx_output_np.astype(np.float32)
                pytorch_output_float = pytorch_output_np.astype(np.float32)
                print(f"PyTorch Output: {pytorch_output_float}")
                print(f"ONNX Output: {onnx_output_float}")
